...

{
    ...
    "category": "section",
    "text": "In this example we will solve the equationfracdudt = f(upt)on the time interval tin01 where f(upt)=u. We know by Calculus that the solution to this equation is u(t)=uexp(t).The general workflow is to define a problem, solve the problem, and then analyze the solution. The full code for solving this problem is:using DifferentialEquations\nf(u,p,t) = 1.01*u\nu0=1/2\ntspan = (0.0,1.0)\nprob = ODEProblem(f,u0,tspan)\nsol = solve(prob,Tsit5(),reltol=1e-8,abstol=1e-8)\nusing Plots\nplot(sol,linewidth=5,title=\"Solution to the linear ODE with a thick line\",\n     xaxis=\"Time (t)\",yaxis=\"u(t) (in μm)\",label=\"My Thick Line!\") # legend=false\nplot!(sol.t, t->0.5*exp(1.01t),lw=3,ls=:dash,label=\"True Solution!\")where the pieces are described below."
},
...

{
    ...
    "category": "section",
    "text": "In this example we will solve the equationdu = f(upt)dt + g(upt)dWwhere f(upt)=u and g(upt)=u. We know via Stochastic Calculus that the solution to this equation isu(tW)=uexp((-frac^22)t+W)To solve this numerically, we define a problem type by giving it the equation and the initial condition:using DifferentialEquations\nα=1\nβ=1\nu₀=1/2\nf(u,p,t) = α*u\ng(u,p,t) = β*u\ndt = 1//2^(4)\ntspan = (0.0,1.0)\nprob = SDEProblem(f,g,u₀,(0.0,1.0))The solve interface is then the same as with ODEs. Here we will use the classic Euler-Maruyama algorithm EM and plot the solution:sol = solve(prob,EM(),dt=dt)\nusing Plots; plotly() # Using the Plotly backend\nplot(sol)(Image: Basic Solution)"
},
...

{
    ...
    "category": "section",
    "text": "We can speed up the previous problem in two different ways. First of all, if we need to interpolate multiple values from a previous time, we can use the in-place form for the history function h(out,t) which writes the output to out. In this case, we must supply the history initial conditions as in-place as well. For the previous example, that's simply:h(out,t) = (out.=1.0)and then our DDE is:const out = zeros(3) # Define a cache variable\nfunction bc_model(du,u,h,p,t)\n  h(out,t-tau) # updates out to be the correct history function\n  du[1] = (v0/(1+beta0*(out[3]^2))) * (p0 - q0)*u[1] - d0*u[1]\n  du[2] = (v0/(1+beta0*(out[3]^2))) * (1 - p0 + q0)*u[1] +\n          (v1/(1+beta1*(out[3]^2))) * (p1 - q1)*u[2] - d1*u[2]\n  du[3] = (v1/(1+beta1*(out[3]^2))) * (1 - p1 + q1)*u[2] - d2*u[3]\nendHowever, we can do something even slicker in most cases. We only ever needed to interpolate past values at index 3. Instead of generating a bunch of arrays, we can instead ask specifically for that value by passing idxs = 3. This must be passed after the derivative. In this case we just want the values so it's the zeroth derivative, i.e. Val{0}. Thus we can instead interpolate the 0th derivative at index 3 via h(t-tau,Val{0},3). The DDE is now:function bc_model(du,u,h,p,t)\n  u3_past_sq = h(t-tau,Val{0},3)^2\n  du[1] = (v0/(1+beta0*(u3_past_sq))) * (p0 - q0)*u[1] - d0*u[1]\n  du[2] = (v0/(1+beta0*(u3_past_sq))) * (1 - p0 + q0)*u[1] +\n          (v1/(1+beta1*(u3_past_sq))) * (p1 - q1)*u[2] - d1*u[2]\n  du[3] = (v1/(1+beta1*(u3_past_sq))) * (1 - p1 + q1)*u[2] - d2*u[3]\nendNote that this requires that we define the historical values:h(t,deriv,idxs) = 1.0where deriv would be Val{0} and idxs is an integer for which variable in the history to compute, and here for any idxs we give back 1.0. Note that if we wanted to use past values of the first derivative then we would define a dispatch like:h(t,::Type{Val{1}},idxs) = 0.0to say that derivatives before t0 are zero for any index.More about the functional forms for the history function are discussed on the DDEProblem page."
},
...

{
    ...
    "title": "Jump Diffusion Equations",
    "category": "section",
    "text": "This tutorial assumes you have read the Ordinary Differential Equations tutorial.Jump Diffusion equations are stochastic diffeential equations with discontinuous jumps. These can be written as:fracdudt = f(upt) + g(ut)dW +  h_i(upt)N_i(t)where N_i is a Poisson-counter which denotes jumps of size h_i. In this tutorial we will show how to solve problems with even more general jumps."
},
...

{
    ...
    "category": "section",
    "text": "To define an Discrete Problem, you simply need to give the function f and the initial condition u which define a function map:u_n+1 = f(upt_n)f should be specified as f(u,p,t) (or in-place as f(du,u,p,t)), and u₀ should be an AbstractArray (or number) whose geometry matches the desired geometry of u. Note that we are not limited to numbers or vectors for u₀; one is allowed to provide u₀ as arbitrary matrices / higher dimension tensors as well.Note that if the discrete solver is set to have scale_by_time=true, then the problem is interpreted as the map:u_n+1 = u_n + dtf(upt_n)"
},
...

{
    ...
    "category": "section",
    "text": "To define an SDE Problem, you simply need to give the forcing function f, the noise function g, and the initial condition u which define an SDE:du = f(upt)dt + g(upt)dWf and g should be specified as f(u,p,t) and  g(u,p,t) respectively, and u₀ should be an AbstractArray whose geometry matches the desired geometry of u. Note that we are not limited to numbers or vectors for u₀; one is allowed to provide u₀ as arbitrary matrices / higher dimension tensors as well. A vector of gs can also be defined to determine an SDE of higher Ito dimension."
},

{
    ...
    "title": "Problem Type",
    "category": "section",
    "text": "Wraps the data which defines an SDE problemu = f(upt)dt + g(upt)dWwith initial condition u0."
},
...

{
    ...
    "category": "section",
    "text": "To define a DDE Problem, you simply need to give the function f and the initial condition u0 which define an ODE:du = f(upth)f should be specified as f(u,h,p,t) (or in-place as f(du,u,h,p,t)). h is the history function which is accessed for all delayed values. For example, the ith component delayed by a time tau is denoted by h(t-tau). Note that we are not limited to numbers or vectors for u0; one is allowed to provide u0 as arbitrary matrices / higher dimension tensors as well."
},

{
    "location": "types/dde_types.html#Functional-Forms-for-h-1",
    "page": "DDE Problems",
...

{
    ...
    "category": "section",
    "text": "To define a DAE Problem, you simply need to give the function f and the initial condition u which define an ODE:0 = f(duupt)f should be specified as f(du,u,p,t) (or in-place as f(resid,u,p,t,du)). Note that we are not limited to numbers or vectors for u₀; one is allowed to provide u₀ as arbitrary matrices / higher dimension tensors as well."
},
...

{
    ...
    "category": "section",
    "text": "AbstractDiffEqOperators act like functions. When defined, A has function calls A(t,u) and A(du,u,p,t) that act like A*u. These operators update via a function update_coefficients!(A,u,p,t)."
},
...

{
    ...
    "category": "section",
    "text": "Function call and multiplication: L(du,u,p,t) for inplace and du = L(t,u) for out-of-place, meaning L*u and A_mul_B!.\nIf the operator is not a constant, update it with (t,u). A mutating form, i.e. update_coefficients!(A,u,p,t) that changes the internal coefficients, and a out-of-place form B = update_coefficients(A,t,u).\nis_constant(A) trait for whether the operator is constant or not."
},
...

{
    ...
    "category": "section",
    "text": "Because the ParameterizedFunction defined by the macro holds the definition at a symbolic level, optimizations are provided by SymEngine. Using the symbolic calculator, in-place functions for many things such as Jacobians, Hessians, etc. are symbolically pre-computed. In addition, functions for the inverse Jacobian, Hessian, etc. are also pre-computed. In addition, parameter gradients and Jacobians are also used.Normally these will be computed fast enough that the user doesn't have to worry. However, in some cases you may want to restrict the number of functions (or get rid of a warning). Macros like @ode_def_nojac turn off the Jacobian calculations, and @ode_def_noinvjac turns off the Jacobian inversion. For more information, please see the ParameterizedFunctions.jl documentation."
},
...

{
    ...
    "category": "section",
    "text": "We can use any DEProblem, which not only includes DAEProblem and DDEProblems, but also stochastic problems. In this case, let's use the generalized maximum likelihood to fit the parameters of an SDE's Monte Carlo evaluation.Let's use the same Lotka-Volterra equation as before, but this time add noise:pf = function (du,u,p,t)\n  du[1] = p[1] * u[1] - u[1]*u[2]\n  du[2] = -3u[2] + u[1]*u[2]\nend\n\nu0 = [1.0;1.0]\npg = function (du,u,p,t)\n  du[1] = p[2]*u[1]\n  du[2] = 1e-2u[2]\nend\np = [1.5,1e-2]\ntspan = (0.0,10.0)\nprob = SDEProblem(pf,pg,u0,tspan,p)Now lets generate a dataset from 10,000 solutions of the SDEusing RecursiveArrayTools # for VectorOfArray\nt = collect(linspace(0,10,200))\nfunction generate_data(t)\n  sol = solve(prob,SRIW1())\n  randomized = VectorOfArray([(sol(t[i]) + .01randn(2)) for i in 1:length(t)])\n  data = convert(Array,randomized)\nend\naggregate_data = convert(Array,VectorOfArray([generate_data(t) for i in 1:10000]))Instead of using UnivariateDistributions like in the previous example, lets fit our data to MultivariateNormal distributions.using Distributions\ndistributions = [fit_mle(MultivariateNormal,aggregate_data[:,j,:]) for j in 1:200]Now let's estimate the parameters. Instead of using single runs from the SDE, we will use a MonteCarloProblem. This means that it will solve the SDE N times to come up with an approximate probability distribution at each time point and use that in the likelihood estimate.monte_prob = MonteCarloProblem(prob)\nobj = build_loss_objective(monte_prob,SOSRI(),LogLikeLoss(t,distributions),\n                                     maxiters=10000,verbose=false,num_monte = 1000,\n                                     parallel_type = :threads)To speed things up I enabled multithreading. Just as before, we hand this over to BlackBoxOptim.jl:using BlackBoxOptim\nbound1 = Tuple{Float64, Float64}[(0.5, 3),(1e-3, 1e-1)]\nresult = bboptimize(obj;SearchRange = bound1, MaxSteps = 400)\n\nOptimization stopped after 201 steps and 2713\n.5920000076294 seconds\nTermination reason: Max number of steps (200)\n reached\nSteps per second = 0.07407156271076672\nFunction evals per second = 0.106869418836429\n59\nImprovements/step = 0.42\nTotal function evaluations = 290\n\n\nBest candidate found: [1.52075, 0.0216393]\n\nFitness: 1544423.794270536Here we see that we successfully recovered the drift parameter, and got close to the original noise parameter after searching a two orders of magnitude range. It would require a larger num_monte to accurately get samples of the the variance and receive a better estimate there."
},
...

{
    ...
    "category": "section",
    "text": "Adjoint sensitivity analysis is used to find the gradient of the solution with respect to some functional of the solution. In many cases this is used in an optimization problem to return the gradient with respect to some cost function.The adjoint requires the definition of some scalar functional g(upt) where u is the (numerical) solution to the differential equation. Adjoint sensitivity analysis finds the gradient ofG(up)=G(u(p))=int_0^Tg(u(tp))dtsome integral of the solution. It does so by solving the adjoint problemfracdlambda^stardt=g_u(t)-lambda^star(t)f_u(t)thinspacethinspacethinspacelambda^star=0where f_u is the Jacobian of the system with respect to the state u while f_p is the Jacobian with respect to the parameters. The adjoint problem's solution gives the sensitivities through the integral:fracdGdp=-int_0^Tlambda^star(t)f_p(t)dt-lambda^T(0)u_p(t)Notice that since the adjoints require the Jacobian of the system at the state, it requires the ability to evaluate the state at any point in time. Thus it requires the continuous forward solution in order to solve the adjoint solution, and the adjoint solution is required to be continuous in order to calculate the resulting integral.There is one extra detail to consider. In many cases we would like to calculate the adjoint sensitivity of some discontinuous functional of the solution. One canonical function is the L2 loss against some data points, that is:L(upt)=sum_i=1^nVerttildeu(t_i)-u(t_ip)Vert^2In this case, we can reinterpret our summation as the distribution integral:G(up)=int_0^Tsum_i=1^nVerttildeu(t_i)-u(t_ip)Vert^2delta(t_i-t)dtwhere  is the Dirac distribution. In this case, the integral is continuous except at finitely many points. Thus it can be calculated between each t_i. At a given t_i, given that the t_i are unique, we have thatg_y(t_i)=2left(tildeu(t_i)-u(t_ip)right)Thus the adjoint solution is given by integrating between the integrals and applying the jump function g_y at every data point."
},
...

{
    ...
    "category": "section",
    "text": "nprob = NBodyProblem(f, mass, pos, vel, tspan)where f is the potential function, mass is the mass matrix, pos and vel are ArrayPartitions for the intial positions and velocity, and tspan is the timespan to solve on."
},
