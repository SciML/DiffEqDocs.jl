...

{
    ...
    "category": "section",
    "text": "In many cases you may want to explicitly have parameters associated with your differential equations. This can be used by things like  parameter estimation routines. In this case, you define a ParameterizedFunction via the syntax:function parameterized_lorenz(t,u,p,du)\n du[1] = p[1]*(u[2]-u[1])\n du[2] = u[1]*(p[2]-u[3]) - u[2]\n du[3] = u[1]*u[2] - p[3]*u[3]\nendand then we can enclose parameters using the constructor:p = [10.0,28.0,8/3]\npf = ParameterizedFunction(parameterized_lorenz,p)and build the ODEProblem from this ParameterizedFunction:u0 = [1.0;0.0;0.0]\ntspan = (0.0,1.0)\nprob = ODEProblem(pf,u0,tspan)Note that the type for the parameters p can be anything: you can use arrays,  static arrays, named tuples, etc. to enclose your parameters in a way that is sensible for your problem.On the ParameterizedFunction page a form of ParameterizedFunction is explained for each type of differential equation, so this method extends throughout the DiffEq-ecosystem.Additionally, there exists a @ode_def macro allows for \"defining your ODE in pseudocode\" and getting a function which is efficient and runnable. To use the macro,  you write out your system of equations with the left-hand side being d_ and those variables will be parsed as the dependent variables. The independent variable is t, and the other variables are parameters which you pass at the end. For example, we can write the Lorenz system as:g = @ode_def LorenzExample begin\n  dx = σ*(y-x)\n  dy = x*(ρ-z) - y\n  dz = x*y - β*z\nend σ=>10.0 ρ=>28.0 β=(8/3)DifferentialEquations.jl will automatically translate this to be exactly the same as f. The result is more legible code with no performance loss. For more information on the macro Domain Specific Language (DSL) and its limitations, please see the parameterized function page The result is that g is a function which you can now use to define the Lorenz problem.u0 = [1.0;0.0;0.0]\ntspan = (0.0,1.0)\nprob = ODEProblem(g,u0,tspan)Since we used =>, σ and ρ are kept as mutable parameters. For example we can do:g.σ = 11.0to change the value of σ to 11.0. β is not able to be changed since we defined it using =. We can create a new instance with new parameters via the name used in the @ode_def command:h = LorenzExample(σ=11.0,ρ=25.0)Note that the values will default to the values given to the @ode_def command.The macro does \"behind-the-scenes\" symbolic calculations to pre-compute things like the Jacobian, inverse Jacobian, etc. in order to speed up calculations. Thus not only will this lead to legible ODE definitions, but \"unfairly fast\" code! We can turn off some of the calculations by using a more specific macro. Here, we can turn off the Jacobian inversion calculations via @ode_def_noinvjac. See ParameterizedFunctions.jl for more details.Since the parameters exist within the function, functions defined in this manner can also be used for sensitivity analysis, parameter estimation routines, and bifurcation plotting. This makes DifferentialEquations.jl a full-stop solution for differential equation analysis which also achieves high performance."
},
...

{
    ...
    "category": "section",
    "text": "For fully implicit ODEs (DAEProblems), a slightly different Jacobian function is necessary. For the DAEG(tudu) = resThe Jacobian should be given in the form dG/du + gamma*dG/d(du) where gamma is given by the solver. This means that the signature is:f(::Type{Val{:jac}},t,u,du,gamma,J)For example, for the equationfunction testjac(t,u,du,res)\n  res[1] = du[1] - 2.0 * u[1] + 1.2 * u[1]*u[2]\n  res[2] = du[2] -3 * u[2] - u[1]*u[2]\nendwe would define the Jacobian as:function testjac(::Type{Val{:jac}},t,u,du,gamma,J)\n  J[1,1] = gamma - 2.0 + 1.2 * u[2]\n  J[1,2] = 1.2 * u[1]\n  J[2,1] = - 1 * u[2]\n  J[2,2] = gamma - 3 - u[1]\n  nothing\nend"
},
...

{
    ...
    "title": "Syntax",
    "category": "section",
    "text": "There are two forms. For discrete adjoints, the form is:s = adjoint_sensitivities(sol,alg,dg,t;kwargs...)where alg is the ODE algorithm to solve the adjoint problem, dg is the jump function, and t is the time points for data. dg is given by:dg(out,u,i)which is the in-place gradient of the cost functional g at time point t[i] with u=u(t[i]).For continuous functionals, the form is:s = adjoint_sensitivities(sol,alg,g,nothing,dg;kwargs...)for the cost functionalg(t,u,p)with in-place gradientdg(out,t,u,p)Currently, the gradient is required. Note that the keyword arguments are passed to the internal ODE solver for solving the adjoint problem. Two special keyword arguments are iabstol and ireltol which are the tolerances for the internal quadrature via QuadGK for the resulting functional."
},
