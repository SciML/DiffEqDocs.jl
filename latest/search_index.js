...
{
    "location": "index.html#Installing-from-Julia-1",
    "page": "Home",
    "title": "Installing from Julia",
    "category": "section",
...
{
    "location": "index.html#Installing-from-Python-1",
    "page": "Home",
    "title": "Installing from Python",
    "category": "section",
    "text": "Use of DifferentialEquations.jl from the Python programming language is available through the diffeqpy module. To install diffeqpy, use pip:pip install diffeqpyUsing diffeqpy requires that Julia is installed and in the path, along with DifferentialEquations.jl and PyCall.jl. To install Julia, download a generic binary from the JuliaLang site and add it to your path. To install Julia packages required for diffeqpy, open up Python interpreter then run:>>> import diffeqpy\n>>> diffeqpy.install()and you\'re good! In addition, to improve the performance of your code it is recommended that you use Numba to JIT compile your derivative functions. To install Numba, use:pip install numbadiffeqpy supports the majority of DifferentialEquations.jl with very similar syntax, see the diffeqpy README for more details. One important point to note is that Numba is generally an order of magnitude slower  than Julia in terms of  the generated differential equation solver code, and thus it is  recommended to use julia.Main.eval for Julia-side derivative function implementations for maximal efficiency. See this blog post for more information."
},

{
    "location": "index.html#Installing-from-R-1",
    "page": "Home",
    "title": "Installing from R",
    "category": "section",
    "text": "Use of DifferentialEquations.jl from the R programming language is available through the  diffeqr module.  diffeqr is registered into CRAN.  Thus to add the package, use:install.packages(\"diffeqr\")To install the master branch of the package (for developers), use:devtools::install_github(\'JuliaDiffEq/diffeqr\', build_vignettes=T)You will need a working installation of Julia in your path. To install Julia, download a generic binary from the JuliaLang site and add it to your path. The download and installation of DifferentialEquations.jl will happen on the first invocation of diffeqr::diffeq_setup().Currently, use from R supported a subset of DifferentialEquations.jl which is documented  through CRAN"
},
...
{
    "location": "tutorials/ode_example.html#Example-3:-Solving-Nonhomogeneous-Equations-using-Parameterized-Functions-1",
    "page": "Ordinary Differential Equations",
    "title": "Example 3: Solving Nonhomogeneous Equations using Parameterized Functions",
    "category": "section",
    "text": "Parameterized functions can also be used for building nonhomogeneous ordinary differential equations (these are also referred to as ODEs with nonzero right-hand sides). They are frequently used as models for dynamical systems with external (in general time-varying) inputs. As an example, consider a model of a pendulum consisting of a slender rod of length l and mass m:beginalign*\nfracmathrmdtheta(t)mathrmdt = omega(t)\nfracmathrmdomega(t)mathrmdt = - frac32fracglsintheta(t) + frac3ml^2M(t)\nendalign*where θ and ω are the angular deviation of the pendulum from the vertical (hanging) orientation and the angular rate, respectively, M is an external torque (developed, say, by a wind or a motor), and finally, g stands for gravitional acceleration.using DifferentialEquations\nusing Plots\n\nl = 1.0                             # length [m]\nm = 1.0                             # mass[m]\ng = 9.81                            # gravitational acceleration [m/s²]\n\nfunction pendulum!(du,u,p,t)\n    du[1] = u[2]                    # θ\'(t) = ω(t)\n    du[2] = -3g/(2l)*sin(u[1]) + 3/(m*l^2)*p(t) # ω\'(t) = -3g/(2l) sin θ(t) + 3/(ml^2)M(t)\nend\n\nθ₀ = 0.01                           # initial angular deflection [rad]\nω₀ = 0.0                            # initial angular velocity [rad/s]\nu₀ = [θ₀, ω₀]                       # initial state vector\ntspan = (0.0,10.0)                  # time interval\n\nM = t->0.1sin(t)                    # external torque [Nm]\n\nprob = ODEProblem(pendulum!,u₀,tspan,M)\nsol = solve(prob)\n\nplot(sol,linewidth=2,xaxis=\"t\",label=[\"θ [rad]\" \"ω [rad/s]\"],layout=(2,1))(Image: Pendulum response)Note how the external time-varying torque M is introduced as a parameter in the pendulum! function. Indeed, as a general principle the parameters can be any type; here we specify M as time-varying by representing it by a function, which is expressed by appending the dependence on time (t) to the name of the parameter.  Note also that, in contrast with the time-varying parameter, the (vector of) state variables u, which is generally also time-varying, is always used without the explicit dependence on time (t)."
},

{
    "location": "tutorials/ode_example.html#Example-4:-Using-Other-Types-for-Systems-of-Equations-1",
    "page": "Ordinary Differential Equations",
    "title": "Example 4: Using Other Types for Systems of Equations",
    "category": "section",
