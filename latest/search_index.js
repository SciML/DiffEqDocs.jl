...

{
    ...
    "category": "section",
    "text": "In this example we will solve the equationfracdudt = f(tu)on the time interval tin01 where f(tu)=u. We know via Calculus that the solution to this equation is u(t)=uexp(t)."
},
...

{
    ...
    "category": "section",
    "text": "DifferentialEquations.jl has a method for choosing the default solver algorithm and the (adaptive) stepsizes dt, and so this will find an efficient method to solve your problem. Because these advanced algorithms may not be known to most users, DifferentialEquations.jl offers a method for choosing algorithms through hints. This default chooser utilizes the precisions of the number types and the keyword arguments (such as the tolerances) to select an algorithm. Additionally one can provide alg_hints to help choose good defaults using properties of the problem and necessary features for the solution. For example, if we have a stiff problem where we need high accuracy, but don't know the best stiff algorithm for this problem, we can use:sol = solve(prob,alg_hints=[:stiff],reltol=1e-8,abstol=1e-8)You can also explicitly choose the algorithm to use. DifferentialEquations.jl offers a much wider variety of solver algorithms than traditional differential equations libraries. Many of these algorithms are from recent research and have been shown to be more efficient than the \"standard\" algorithms (which are also available). For example, we can choose a 7th order Verner Efficient method:sol = solve(prob,Vern7())For users familiar with MATLAB/Python/R, good translations of the standard library methods are as follows:ode23 –> BS3()\node45/dopri5 –> DP5(), though in most cases Tsit5() is more efficient\node23s –> Rosenbrock23()\node113 –> CVODE_Adams(), though in many cases Vern7() is more efficient\ndop853 –> DP8(), though in most cases Vern7() is more efficient\node15s/vode –> CVODE_BDF(), though in many cases radau() is more efficient\node23t –> Trapezoid()\nlsoda –> lsoda() (requires Pkg.add(\"LSODA\"); using LSODA)\node15i –> IDA()In DifferentialEquations.jl, some good \"go-to\" choices for ODEs are:BS3() for fast low accuracy non-stiff\nTsit5() for non-stiff\nVern7() for high accuracy non-stiff\nRosenbrock23() for stiff equations with Julia-defined types, events, etc.\nCVODE_BDF() or radau() for stiff equations of Vector{Float64}Note that radau() requires the installation of ODEInterfaceDiffEq:Pkg.add(\"ODEInterfaceDiffEq\")\nusing ODEInterfaceDiffEqThis solve interface, known as the common interface, is actually pooling together the methods from many different packages into a single API. For a comprehensive list of the available algorithms and detailed recommendations, Please see the solver documentation. Every problem type has an associated page detailing all of the solvers associated with the problem. In many cases, you may not need the full functionality of all of the packages together and may want to reduce the size of your dependency. To find out how to require only the parts of DifferentialEquations.jl you're specifically using, see the low dependency usage manual page.Note that the solver controls can be combined with the algorithm choice. Thus we can for example solve the problem using Vern7() with a lower tolerance via:sol = solve(prob,Vern7(),reltol=1e-8,abstol=1e-8)"
},
...

{
    ...
    "category": "section",
    "text": "In this example we will solve the Lorenz equations:  fracdxdt = (y-x) \n  fracdydt = x(-z) - y \n  fracdzdt = xy - z Defining your ODE function to be in-place updating can have performance benefits. What this means is that, instead of writing a function which outputs its solution, you write a function which updates a vector that is designated to hold the solution. By doing this, DifferentialEquations.jl's solver packages are able to reduce the amount of array allocations and achieve better performance.The way we do this is we simply write the output to the 3rd input of the function. For example, our Lorenz equation problem would be defined by the function:function lorenz(t,u,du)\n du[1] = 10.0(u[2]-u[1])\n du[2] = u[1]*(28.0-u[3]) - u[2]\n du[3] = u[1]*u[2] - (8/3)*u[3]\nendand then we can use this function in a problem:u0 = [1.0;0.0;0.0]\ntspan = (0.0,100.0)\nprob = ODEProblem(lorenz,u0,tspan)\nsol = solve(prob)Using the plot recipe tools defined on the plotting page, we can choose to do a 3D phase space plot between the different variables:plot(sol,vars=(1,2,3))(Image: Lorenz System)Note that the default plot for multi-dimensional systems is an overlay of each timeseries. We can plot the timeseries of just the second component using the variable choices interface once more:plot(sol,vars=(0,2))(Image: Lorenz Timeseries)Note that here \"variable 0\" corresponds to the dependent variable (\"time\")."
},
