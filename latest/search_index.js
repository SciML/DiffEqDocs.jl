...

{
    ...
    "category": "section",
    "text": "When energy conservation is required, use a symplectic method. Otherwise the Runge-Kutta Nystrom methods will be more efficient. Energy is mostly conserved by Runge-Kutta Nystrom methods, but is not conserved for long time integrations. Thus it is suggested that for shorter integrations you use Runge-Kutta Nystrom methods as well.As a go-to method for efficiency, DPRKN6 is a good choice. DPRKN12 is a good choice when high accuracy, like tol<1e-10 is necessary. However, DPRKN6 is the only Runge-Kutta Nystrom method with a higher order interpolant (all default to order 3 Hermite, whereas DPRKN6 is order 6th interpolant) and thus in cases where interpolation matters (ex: event handling) one should use DPRKN6. For very smooth problems with expensive acceleration function evaluations, IRKN4 can be a good choice as it minimizes the number of evaluations.For symplectic methods, higher order algorithms are the most efficient when higher accuracy is needed, and when less accuracy is needed lower order methods do better. Optimized efficiency methods take more steps and thus have more force calculations for the same order, but have smaller error. Thus the \"optimized efficiency\" algorithms are recommended if your force calculation is not too sufficiency large, while the other methods are recommend when force calculations are really large (for example, like in MD simulations VelocityVerlet is very popular since it only requires one force calculation per timestep). A good go-to method would be McAte5, and a good high order choice is KahanLi8."
},
...

{
    ...
    "category": "section",
    "text": "Nystrom4: 4th order explicit Runge-Kutta Nystrom method. Allows acceleration to depend on velocity. Fixed timestep only.\nIRKN3: 4th order explicit two-step Runge-Kutta Nystrom method. Fixed timestep only.\nIRKN4: 4th order explicit two-step Runge-Kutta Nystrom method. Can be more efficient for smooth problems. Fixed timestep only.\nNystrom4VelocityIndependent: 4th order explicit Runge-Kutta Nystrom method. Fixed timestep only.\nNystrom5VelocityIndependent: 5th order explicit Runge-Kutta Nystrom method. Fixed timestep only.\nDPRKN6: 6th order explicit adaptive Runge-Kutta Nystrom method. Free 6th order interpolant.\nDPRKN8: 8th order explicit adaptive Runge-Kutta Nystrom method.\nDPRKN12: 12th order explicit adaptive Runge-Kutta Nystrom method."
},
...

{
    ...
    "title": "Stiff Methods",
    "category": "section",
    "text": "ImplicitEM - An order 0.5 Ito implicit method. This is a theta method which defaults to theta=1/2 or the Trapezoid method on the drift term. This method defaults to symplectic=false, but when true and theta=1/2 this is the implicit Midpoint method on the drift term and is symplectic in distribution.\nImplicitEulerHeun - An order 0.5 Stratonovich implicit method. This is a theta method which defaults to theta=1/2 or the Trapezoid method on the drift term. This method defaults to symplectic=false, but when true and theta=1/2 this is the implicit Midpoint method on the drift term and is symplectic in distribution.\nImplicitRKMil - An order 1.0 implicit method. This is a theta method which defaults to theta=1/2 or the Trapezoid method on the drift term. Defaults to solving the Ito problem, but ImplicitRKMil(interpretation=:Stratonovich) makes it solve theStratonovich problem. This method defaults to symplectic=false, but when true and theta=1/2 this is the implicit Midpoint method on the drift term and is symplectic in distribution."
},
