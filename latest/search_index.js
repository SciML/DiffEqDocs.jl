...

{
    ...
    "title": "Extra Options",
    "category": "section",
    "text": "The following methods allow for specification of linsolve: the linear solver which is used:ImplicitEuler\nTrapezoid\nTRBDF2\nRosenbrock23\nRosenbrock32\nROS3P\nRodas3\nRosShamp4\nVeldd4\nVelds4\nGRK4T\nGRK4A\nRos4LStab\nRodas4\nRodas42\nRodas4P\nRodas5For more information on specifying the linear solver, see the manual page on solver specification.The following methods allow for specification of nlsolve: the nonlinear solver which is used:GenericImplicitEuler\nGenericTrapezoidNote that performance overload information (Jacobians etc.) are not used in this mode. This can control autodifferentiation of the Jacobian as well. For more information on specifying the nonlinear solver, see the manual page on solver specification.Additionally, the following methods have extra differentiation controls:ImplicitEuler\nTrapezoid\nTRBDF2\nRosenbrock23\nRosenbrock32\nROS3P\nRodas3\nRosShamp4\nVeldd4\nVelds4\nGRK4T\nGRK4A\nRos4LStab\nRodas4\nRodas42\nRodas4P\nRodas5In each of these, autodiff can be set to turn on/off autodifferentiation, and chunk_size can be used to set the chunksize of the Dual numbers (see the documentation for ForwardDiff.jl for details). In addition, the Rosenbrock methods can set diff_type, which is the type of numerical differentiation that is used (when autodifferentiation is disabled). The choices are :central or :forward.Examples:sol = solve(prob,Rosenbrock23()) # Standard, uses autodiff\nsol = solve(prob,Rosenbrock23(chunk_size=10)) # Autodiff with chunksize of 10\nsol = solve(prob,Rosenbrock23(autodiff=false)) # Numerical differentiation with central differencing\nsol = solve(prob,Rosenbrock23(autodiff=false,diff_type=:forward)) # Numerical differentiation with forward differencing"
},
